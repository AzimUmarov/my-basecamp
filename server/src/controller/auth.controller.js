...
class AuthController {
    async login(req, res) {
        ...
        await jwt.verify(existingUser.password, password, (err, user) => {
            if (err)
        ...
        try {
            await jwt.sign(name, password, async (err, token) => {
    ...
    async register(req, res){
        ...
        try {
        if (passToken) {
            const user = new User({name, email, password: passToken});
            const newUser =  await user.save().catch(err => console.log(err.message));
            ...
            jwt.sign(name, password, async (err, token) => {
                ...
                await client.connect();
                await client.set(token, token);
                res.status(200).json({message: "user created", token: token, newUser});
            });
        }
        }
        catch (err) {
