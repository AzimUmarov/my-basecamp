...
function Discussion({discussion}) {
    ...
    const handleMessageAction = async (event, what) => {
        ...
        const data = new FormData(event.currentTarget);
            ...
            try {
                const response = await ServiceAPI.post(`/discussion/${discussion?._id}/message/create`,
    ...
    return (
        <>
            ...
            <Box component="form" noValidate onSubmit={handleMessageAction} sx={{ml: 9, mt: 3, alignItems: 'end'}}>
                        <TextField
                            name="title"
                            label="Your Message"
                            type="text"
